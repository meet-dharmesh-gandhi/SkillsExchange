...
export default function DashboardPage() {
	...
	useEffect(() => {
		if (!user) return;
		apiGet(`/user-skills/${user.user_id}`).then(setSkills).catch(() => {});
		apiGet(`/user-requests/${user.user_id}`).then(setRequests).catch(() => {});
		apiGet(`/sessions/${user.user_id}`).then(setSessions).catch(() => {});
	}, [user]);
			...
			<div className="grid gap-6 lg:grid-cols-2">
				...
				<div className="card">
					<div className="mb-3 flex items-center justify-between">
						<h2>My Skills</h2>
						<Link href="/skills/my-skills" className="text-xs font-medium text-teal-700">
							View all →
						</Link>
					</div>
					{skills.length === 0 ? (
					...
					{upcomingSessions.length === 0 ? (
						...
						<div className="flex flex-col gap-2">
							{upcomingSessions.slice(0, 5).map((s) => {
								...
								return (
									<div key={s.session_id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
										<div>
											<span className="text-sm font-medium text-slate-700">with {otherName}</span>
											<p className="text-xs text-slate-400">{time}</p>
										</div>
										<span className={`badge ${s.status === "upcoming" ? "badge-emerald" : "badge-amber"}`}>
											{s.status}
										</span>
									</div>
								);
					...
					{pendingRequests.length === 0 ? (
						...
						<div className="flex flex-col gap-2">
							{pendingRequests.slice(0, 5).map((r) => (
								<div key={r.request_id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
